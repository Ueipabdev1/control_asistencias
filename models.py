...
# Modelo para secciones (V2 - tabla singular)
class Seccion(db.Model):
    ...
    def nombre_completo(self):
        if self.grado:
            return f"{self.grado.nombre_grado} {self.nombre_seccion}"
        return self.nombre_seccion

    def __repr__(self):
        return f'<Seccion {self.nombre_seccion} - Grado:{self.id_grado}>'

# Modelo para relación profesor-secciones (V2 - tabla singular)
...
# FK apunta a secciones V1 (tabla plural)
...
# Modelo para estudiantes individuales (V2)
...
# Modelo para asistencia individual por estudiante (V2)
...
# Modelo para observaciones generales de sección (V2 - FK a seccion)
...
# Tabla V1 secciones (plural) - necesaria para FK de Matricula
class SeccionLegacy(db.Model):
    ...
    etapa = db.relationship('Etapa', backref='secciones_legacy', lazy=True)
